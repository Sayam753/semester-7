...
tf.app.flags.DEFINE_string(
    "train_dir", "./train_color", """Directory where to write summaries and checkpoint."""
)
tf.app.flags.DEFINE_string(
    "base_dir", "./data/ShapeNetCore_im2avatar", """The path containing all the samples."""
)
tf.app.flags.DEFINE_string(
    "cat_id",
    "02958343",
...
tf.app.flags.DEFINE_string("data_list_path", "./data_list", """The path containing data lists.""")
    ...
    with tf.Graph().as_default():
        with tf.device("/gpu:" + str(GPU_INDEX)):
            ...
            pred_reg_clr, pred_conf, pred_flow, pred_blended_clr = model.get_model(
                img_pl, is_train_pl, weight_decay=FLAGS.wd, bn_decay=bn_decay
            )
            loss = model.get_loss(
                pred_reg_clr, pred_blended_clr, vol_clr_pl, pred_flow, vol_flow_pl
            )
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(TRAIN_EPOCHS):
                ...
                for batch_idx in range(train_batches):
                    ...
                    log_string(
                        "<TRAIN> Epoch {} - Batch {}: loss: {}.".format(epoch, batch_idx, loss_val)
                    )
                ...
                for batch_idx in range(val_batches):
                    imgs, vols_flow, vols_clr = dataset_.next_flow_batch(
                        batch_idx * BATCH_SIZE, BATCH_SIZE, vol_dim=VOL_DIM, process="val"
                    )
                    feed_dict = {
                        img_pl: imgs,
                        vol_clr_pl: vols_clr,
...
def main():
    train_dataset = dataset.Dataset(
        base_path=FLAGS.base_dir, cat_id=FLAGS.cat_id, data_list_path=FLAGS.data_list_path
    )

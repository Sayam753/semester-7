...
def get_loss(regressed_clr, blended_clr, target_clr, pred_flow, target_flow):
    ...
    regressed_clr_pos_loss = tf.where(
        target_clr > -0.5, regressed_clr_vol_loss, tf.zeros_like(regressed_clr_vol_loss)
    )
    ...
    blended_clr_pos_loss = tf.where(
        target_clr > -0.5, blended_clr_vol_loss, tf.zeros_like(blended_clr_vol_loss)
    )
...
def get_model(imgs, is_training, weight_decay=0.0, bn_decay=None):
    ...
    with tf.variable_scope("Decoding"):
        ...
        net_reg_clr = tf_utils.conv3d_transpose(
            net,
            16,
            [3, 3, 3],
            scope="deconv_reg_clr1",
            stride=[2, 2, 2],
            padding="SAME",
            weight_decay=weight_decay,
            activation_fn=tf.nn.elu,
        ...
        net_reg_clr = tf_utils.conv3d(
            net_reg_clr,
            3,
            [3, 3, 3],
            scope="deconv_reg_clr2",
            stride=[1, 1, 1],
            padding="SAME",
            weight_decay=weight_decay,
        ...
        ### confidence
        ##############
        net_conf = tf_utils.conv3d_transpose(
            net,
            16,
            [3, 3, 3],
            scope="deconv_conf1",
        ...
        ### flow color
        ##############
        net_flow = tf_utils.conv3d_transpose(
            net,
            2,
            [3, 3, 3],
            scope="deconv_flow",

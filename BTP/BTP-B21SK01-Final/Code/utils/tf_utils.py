...
def conv1d(
    ...
    with tf.variable_scope(scope) as sc:
        ...
        kernel = _variable_with_weight_decay(
            "weights", shape=kernel_shape, use_xavier=use_xavier, stddev=stddev, wd=weight_decay
        )
        ...
        if bn:
            outputs = batch_norm_for_conv1d(
                outputs, is_training, bn_decay=bn_decay, scope="bn", is_dist=is_dist
            )
...
def conv3d_transpose(
    ...
    with tf.variable_scope(scope) as sc:
        ...
        outputs = tf.nn.conv3d_transpose(
            inputs, kernel, output_shape, [1, stride_h, stride_w, stride_d, 1], padding=padding
        )
        biases = _variable_on_cpu("biases", [num_output_channels], tf.constant_initializer(0.0))
        outputs = tf.nn.bias_add(outputs, biases)
...
def batch_norm_dist_template(inputs, is_training, scope, moments_dims, bn_decay):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        pop_mean = _variable_on_cpu(
            "pop_mean", [num_channels], initializer=tf.zeros_initializer(), trainable=False
        )
        pop_var = _variable_on_cpu(
            "pop_var", [num_channels], initializer=tf.ones_initializer(), trainable=False
        )
...
def batch_norm_for_fc(inputs, is_training, bn_decay, scope, is_dist=False):
    ...
    """
    if is_dist:
        return batch_norm_dist_template(
            inputs,
            is_training,
            scope,
            [
...
def dropout(inputs, is_training, scope, keep_prob=0.5, noise_shape=None):
    ...
    with tf.variable_scope(scope) as sc:
        outputs = tf.cond(
            is_training, lambda: tf.nn.dropout(inputs, keep_prob, noise_shape), lambda: inputs
        )
        return outputs
...
def upsample_vol(vol, scales=[2, 2, 2]):
    ...
    """
    batch_size = vol.get_shape()[0].value
    vol_dim = vol.get_shape()[1].value
    vol_chan = vol.get_shape()[4].value
    ...
    vol_rep_width = tf.reshape(
        vol_rep_width, (batch_size, vol_dim, vol_dim, vol_dim * repeat_width, vol_chan)
    )

    vol_rep_height = tf.tile(
        tf.reshape(vol_rep_width, (-1, vol_dim * repeat_width, vol_chan)), (1, repeat_height, 1)
    )
    vol_rep_height = tf.reshape(
        vol_rep_height,
        (batch_size, vol_dim, vol_dim * repeat_height, vol_dim * repeat_width, vol_chan),
    )

    vol_rep_depth = tf.tile(
        tf.reshape(
            vol_rep_height, (-1, vol_dim * repeat_height, vol_dim * repeat_width, vol_chan)
        ),
        (1, repeat_depth, 1, 1),
    )
    vol_rep_depth = tf.reshape(
        vol_rep_depth,
        (
            batch_size,
            vol_dim * repeat_depth,
            vol_dim * repeat_height,
            vol_dim * repeat_width,
            vol_chan,
        ),
    )
...
def Sampler(fields, images):
    ...
    batch = fields.get_shape()[0].value
    fields.get_shape()[1].value
    im_height = images.get_shape()[1].value
    im_width = images.get_shape()[2].value
    fields.get_shape()[-1].value
    im_dim = images.get_shape()[-1].value

...
sheep-own [
  attacked
;  heading-direction-after-attack
]
...
to setup
  ...
  create-sheep number-of-sheep [  ;; create the initial sheep
    ...
    set attacked 0
;    set heading-direction-after-attack 0
  ]
...
to eat-sheep [direction-of-wolf]
  let target-sheep one-of sheep in-radius wolf-vision-range
  if target-sheep != nobody [
    ask target-sheep [
      set attacked 1
;      set heading-direction-after-attack direction-of-wolf
    ]
  ]
...
to reproduce
  if energy > 200 [
    ...
    hatch 1 [
      set energy 100
      if breed = wolves [
        let corner-location random 4
        if corner-location = 0 [
          setxy max-pxcor max-pycor
        ]
        if corner-location = 1 [
          setxy max-pxcor min-pycor
        ]
        if corner-location = 2 [
          setxy min-pxcor max-pycor
        ]
        if corner-location = 3 [
...
to snow-updates
  ask patches [
    ifelse pcolor = white [
      set snow-amount snow-amount + random-normal 2 5
    ]
    [
      ifelse snow-amount > snow-amount-threshold [
        set snow-amount random 100
        set pcolor white
      ]
      [
        ; increase snow to reach the threshold
        set snow-amount snow-amount + random 2
      ]
    ]

...
to go
  ask bots [
    ...
    ask current-node [ if color = yellow [
...
to-report find-target [current-node]
  ...
  let sort-link-neighbors sort-on [visited] neighbor-nodes
  report first sort-link-neighbors
  ; report one-of neighbor-nodes
end
